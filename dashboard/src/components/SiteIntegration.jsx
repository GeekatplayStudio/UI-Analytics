    ...
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(360px, 1fr))', gap: '20px' }}>
      ...
      <div className="glass-card fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
        ...
        <div style={{ background: 'rgba(0,0,0,0.15)', padding: '10px 14px', borderRadius: '8px', border: '1px solid var(--border-color)', fontSize: '12px' }}>
          {latestSession ? (
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span>Testing active session: <strong style={{ color: 'var(--accent-secondary)' }}>{latestSession.id.startsWith('walkthrough-') ? `Walkthrough: ${latestSession.id.substring(12, 22)}` : latestSession.id.substring(0, 8) + '...'}</strong></span>
